...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('price_catalog', table => {
    ...
    table
      ...
      .nullable()
      .references('id')
      .inTable('price_sources')
      .onDelete('SET NULL');
    table.string('source_type', 50).nullable();

    // Метаданные
    table.decimal('confidence_score', 3, 2).defaultTo(0.5);
    ...
    table.index(['name', 'city', 'category']);
  ...
  await knex.schema.createTable('price_history', table => {
    table.string('id', 36).primary();
    table
      .string('price_catalog_id', 36)
      .notNullable()
      .references('id')
      .inTable('price_catalog')
      .onDelete('CASCADE');
