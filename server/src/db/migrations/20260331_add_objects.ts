...
async function migrateExistingData(knex: Knex): Promise<void> {
  ...
  for (const user of users) {
    ...
    for (const oldProject of oldProjects) {
      ...
      const roomsResult = await knex('rooms').where('project_id', oldProject.id).update({
        object_id: objectId,
        updated_at: knex.fn.now(),
      });
  ...
  console.log(
    `[MIGRATION] Завершено: ${migratedProjectsCount} проектов, ${migratedRoomsCount} комнат`,
  );
}
...
export async function down(knex: Knex): Promise<void> {
  ...
  if (knex.client.config.client === 'pg') {
    await knex.raw(`
      UPDATE rooms 
      SET project_id = objects.project_id
      FROM objects 
      WHERE rooms.object_id = objects.id
    `);
    await knex.raw(`
      UPDATE projects 
      SET deleted_at = NULL
      FROM objects 
      WHERE projects.id = objects.project_id AND projects.name = 'Мои объекты'
    `);
  } else {
    await knex.raw(`
      UPDATE rooms r
