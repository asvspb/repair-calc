/**
 * Gemini AI Provider - для оценки работ и подбора материалов
...
/**
 * Gemini AI Provider - реализация интерфейса AIProvider
 */
export class GeminiAIProvider extends BaseAIProvider implements AIProvider {
  ...
  /**
   * Выполнение запроса к Gemini API
   */
  ...
  private extractText(data: unknown): string {
    ...
    if (!text) {
      throw new AIProviderError('Пустой ответ от Gemini API', this.name, false, 'EMPTY_RESPONSE');
    }
...
Верни JSON в точном формате (БЕЗ markdown обёртки):
{
  "works": [
    {
...
/**
 * Проверяет, включён ли Gemini AI
 */
export function isGeminiAIEnabled(): boolean {
  return !!getApiKey();
}

// Singleton instance
let geminiAIProviderInstance: GeminiAIProvider | null = null;

export function getGeminiAIProvider(): GeminiAIProvider {
  if (!geminiAIProviderInstance) {
    geminiAIProviderInstance = new GeminiAIProvider();
  }
  return geminiAIProviderInstance;
}