...
- **AI поиск цен** — Gemini и Mistral API через серверный прокси (ключи не попадают в клиентский бандл)
...
2. Настроить API-ключи (серверные, для AI поиска цен):
   ...
   Ключи задаются на сервере и **не** попадают в клиентский бандл.
...
| `npm run build`    | Production сборка в папку `dist` (только для анализа, не для деплоя!) |
...
Для деплоя приложения используйте единый скрипт:

```bash
./scripts/deploy-local.sh
```

Он автоматически проверяет тесты, линтер и собирает Docker-образы с актуальным хешем коммита. Локальный билд через `npm run build` не связан с контейнерами и используется только для отладки.

---

## Docker

Docker-конфигурации используются для продакшен-окружения через `deploy-local.sh`.
